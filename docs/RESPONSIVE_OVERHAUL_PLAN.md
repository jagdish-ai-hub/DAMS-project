...
| **BUG-6** | `ui.tsx:211-254`<br>`AddPaymentModal.tsx:140-146` | `<div style={{ padding: '18px 20px', ... overflowY: 'auto' }}>{props.children}</div>` | In `Modal`, the content wrapper placed all `{props.children}` inside a single scrollable area without `flex: 1, minHeight: 0`, and modal actions (Cancel / Add Payment) were appended at the end. When form fields or small viewports reduced height, action buttons were pushed below the viewport boundary and clipped. | Added explicit `footer?: ReactNode` slot to `Modal` pinned with `flexShrink: 0`, centered modal vertically, and passed buttons via `footer={...}` in `AddPaymentModal.tsx`. | `a342e51` |
| **BUG-7** | `AppShell.tsx:93-96` | `<header style={{ background: 'var(--navy)', ... height: 52 }}>` | Navigation `<header>` had static positioning. On long forms like `NewReceiptPage` and `NewExpensePage` or tall tables, scrolling down caused the entire navbar (brand, navigation links, user avatar) to scroll off-screen. | Added `position: 'sticky', top: 0, zIndex: 40` to `<header>` style so the navigation bar remains pinned on scroll. | `a342e51` |
| **BUG-8** | `NewReceiptPage.tsx:325-334`<br>`JobCardPatchRequest.java:22-41`<br>`JobCardService.java:138-184` | `await jobCardsApi.patch(loadedDoc.jobCardId, { invoiceNo, invoiceAmount, dbmId, b2b, gstNo, categoryId, businessStatusId })` | `NewReceiptPage` allows editing `Vehicle #` and invoice amount. On `saveDraftExisting()`, `patchHeader()` did not pass `vehicleNo`, and `JobCardPatchRequest` had no `vehicleNo` field, silently dropping vehicle edits. Additionally, clearing `invoiceAmount` sent `undefined` (omitted from JSON), preventing clearing. | Added `vehicleNo` and `clearInvoiceAmount` to `JobCardPatchRequest.java`, handled vehicle normalization and association in `JobCardService.java`, and passed `vehicleNo` and `clearInvoiceAmount` from `NewReceiptPage.tsx`. | `a342e51` |
| **BUG-9** | `NewReceiptPage.tsx:378-401`<br>`NewExpensePage.tsx:417-435` | `await syncLines(...)`<br>`finish(false, loadedDoc)`<br>`navigate('/app?flash=...')` | Saving an existing draft navigated away to `/app` (CashierHomePage) where drafts are not displayed, causing cashier confusion. In addition, if a line amount was blank/0 on draft save, `syncLines` sent `amount: 0`, triggering an HTTP 400 validation rejection from the backend. | Replaced navigation in `saveDraftExisting()` with in-place document reload (`receiptsApi.get(id)`), updated local `loadedDoc` and indexed lines, rendered success notification banner, and guarded `updateLine` against non-positive amounts. | `a342e51` |
| **BUG-10** | `NewReceiptPage.tsx:236-248`<br>`NewExpensePage.tsx:275-288`<br>`CashPage.tsx:354, 442`<br>`ViewReceiptsModal.tsx:71` | `finish(submit, data)`<br>`setLines((prev) => (prev.length > 1 ? ... : prev))` | Saving new receipt/expense as draft redirected to `/app` losing view of draft; `removeLine` prevented deleting the single remaining line; `syncLines` didn't delete cleared lines on server; and cash drawer/receipt modal action buttons were placed in body instead of pinned `footer`. | Stay on page in `saveNew(false)` and `ensureDraft` with `?editDoc=${data.id}`; map `data.lines` to `lines`; guard `useEffect` from re-fetching when `loadedDoc?.id === editDocId`; reset single line on `removeLine`; filter `kept` in `syncLines` to delete cleared lines on server; and use `Modal footer={...}` in `MovementModal`, `CloseDayModal`, and `ViewReceiptsModal`. | `328b863` |
| **BUG-11** | `CashDocumentService.java:168-174`<br>`ReceiveDocumentService.java:310-318`<br>`ExpenseDocumentService.java:311-320` | `if (request.getTransactionDate() != null) { ... }`<br>`applyLineInput(...)` | In `CashDocumentService.patch()`, date lock check was skipped if `transactionDate` was omitted in the patch body. In `ReceiveDocumentService.updateLine()` and `ExpenseDocumentService.updateLine()`, `applyLineInput()` only validated the new date/mode, bypassing date locking on the existing line if it was originally recorded as cash on an already closed date. | Enforced `cashDateLock.requireCashDateOpen(orgId, doc.getBranchId(), doc.getTransactionDate())` on existing document date, and verified `cashDateLock.requireCashLineDateOpen()` on existing settlement/expense lines before applying updates. | `ce52a28` |
| **BUG-12** | `CashCloseService.java:112-117, 152-160`<br>`CashDayCloseRepository.java:14-16`<br>`CashPage.tsx:128-136, 431` | `closeDay(request)` allowed closing without `openingSet` | Cashier could close cash day when `p.openingSet() == false`, locking branch opening to 0 and poisoning future opening calculations. In addition, `setOpening()` did not verify if the branch already recorded daily closes. | Required `p.openingSet()` in `closeDay()`, guarded `setOpening()` against branches with existing day closes (`cashDayCloseRepo.existsByOrgIdAndBranchId`), disabled "Close Day" button on `CashPage.tsx` when `!drawer.openingSet`, and added modal confirmation guard. | `ce52a28` |
| **BUG-13** | `AttachmentService.java:126-132, 190-194`<br>`AddPaymentModal.tsx:68-76` | `private static OwningDoc receiveOwner(ReceiveDocument doc) { boolean frozen = doc.isSettled() ... }` | When adding payment in `AddPaymentModal` that pays off remaining balance, `ReceiveDocumentService.addLine()` auto-settled the document. Immediately following, `attachToLine()` failed with 409 Conflict because `receiveOwner()` marked settled documents frozen, silently dropping customer receipt slips. | Updated `receiveOwner()` to freeze on `WorkflowStatus.APPROVED` or `REJECTED`, allowing receipt attachment uploads during payment addition and draft/submitted review phases. In `AttachmentService.delete()`, registered `TransactionSynchronization.afterCommit()` to safely delete storage files only after DB commit succeeds. | `ce52a28` |
| **BUG-14** | `CashDocumentService.java:175-184`<br>`CashDocumentPatchRequest.java:28-35`<br>`CashPage.tsx:320-323`<br>`cash.ts:81-86` | `if (request.getBankId() != null)`<br>`bankId: bankId === '' ? null : Number(bankId)` | Selecting "— none —" for bank in `MovementModal` sent `{ bankId: null }`, which the backend patch logic skipped because of `!= null`, making it impossible to unset a bank on a cash movement. | Added `clearBank` boolean flag to `CashDocumentPatchRequest`, handled `isClearBank()` in `CashDocumentService.patch()`, and passed `clearBank: edit ? bankId === '' : undefined` from `CashPage.tsx`. | `ce52a28` |
| **BUG-15** | `ReceiveDocumentService.java:250-255` | `addLine(Long documentId, SettlementLineInput input)` | While `create()` prevented opening a receipt on a closed claim, `addLine()` lacked an explicit check for `claimCloseRepo.existsByOrgIdAndJobCardId()`. | Added explicit `claimCloseRepo.existsByOrgIdAndJobCardId(orgId, jobCard.getId())` guard in `ReceiveDocumentService.addLine()` as defense-in-depth to guarantee claim finality. | `ce52a28` |
...
| `a342e51` | `shell/ui.tsx`<br>`cashier/AddPaymentModal.tsx`<br>`shell/AppShell.tsx`<br>`backend/.../JobCardPatchRequest.java`<br>`backend/.../JobCardService.java`<br>`frontend/src/api/jobCards.ts`<br>`cashier/NewReceiptPage.tsx`<br>`cashier/NewExpensePage.tsx` | `fix(modals,drafts): resolve modal button clipping, sticky header, vehicleNo patch, and in-place draft save` | Fixes BUG-6, BUG-7, BUG-8, BUG-9 |
