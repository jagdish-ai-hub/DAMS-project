            ...
            <div style={{ ...card }}>
              ...
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={summary.trend.map((t) => ({ ...t, day: t.date.slice(8) }))}>
                  ...
                  <YAxis tick={{ fontSize: 11, fill: 'var(--faint)' }} tickFormatter={(v) => `₹${Math.round(v / 1000)}k`} />
                  <Tooltip formatter={(v) => inr(Number(v))} />
                  <Area type="monotone" dataKey="collections" stroke="var(--green)" fill="var(--green-bg)" strokeWidth={2} name="Collections" />
                  <Area type="monotone" dataKey="expenses" stroke="var(--red)" fill="#FDE8E8" strokeWidth={2} name="Expenses" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
