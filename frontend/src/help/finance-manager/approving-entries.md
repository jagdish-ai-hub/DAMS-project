# Approving entries

Your home screen is **Approvals & Claims**. It lists every entry the accountant has verified and passed up to you for the final decision. Nothing counts as real money in the business until you approve it here.

## Work an entry

1. Use the toggle to switch between **Receipts**, **Expenses** and **Cash**.
2. Under **Awaiting final approval**, click an entry. The right pane shows its header, every line (with any accountant override struck through), and the full history.
3. Decide:
   - **Approve** — the entry is final. It now shows in the owner's dashboard. For an over-limit expense, your approval is also what lets the accountant close it.
   - **Query** — send it back with a question. It returns to the cashier, then through the accountant, then back to you.
   - **Reject** — the entry shouldn't stand. Give a reason; a rejected entry is closed.


## Good to know

- You **can't approve an entry you created or last changed** — someone else approves that one.
- **Approving accepts every override on the record** as final. If the accountant changed a line, you see the original struck through, the new figure, and their reason right there — check it before you approve.
- Every override also appears on the **Override Audit** screen.
- You see **all branches** — there's no branch limit on your role.
